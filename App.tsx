...
function App() {
    ...
    const handleAddProfile = async () => {
        ...
        const newId = await firestoreService.saveProfile(newProfile);
        const createdProfile = { ...newProfile, id: newId, createdAt: new Date() } as Profile; // Approximate timestamp
    ...
    // Onboarding
    const handleOnboardingFinish = async (result: { resume: string, settings: SearchSettings }) => {
        if (!user) return;
        const newProfileData: Omit<Profile, 'id' | 'createdAt'> = {
            userId: user.uid,
            name: `Профиль ${result.settings.positions.split(',')[0]}`,
    ...
    const handleGenerateEmail = async (job: Job) => {
        ...
        setIsModalLoading(true);
        try {
            const candidateName = activeProfile.name || 'Кандидат';
                    ...
                    {activeProfile ? (
                        <StatusBar status={status} message={message} />
                         ...
                         <div className="text-center p-8 bg-white dark:bg-slate-800 rounded-lg shadow-md">
                            <h3 className="text-xl font-semibold">Профиль не найден</h3>
                            <p className="text-slate-500 dark:text-slate-400 mt-2">Создайте новый профиль, чтобы начать поиск.</p>
                             <button onClick={handleAddProfile} className="mt-4 px-4 py-2 bg-primary-600 text-white font-semibold rounded-md hover:bg-primary-700">
                                Создать профиль
                            </button>
                        </div>
                    )}
